      ...
      <div className="min-h-0 flex-1 overflow-y-auto pb-2">
        {workspaceStatus === "loading" && openActions.length === 0 ? (
          ...
          openActions.map((action) => (
            <div
              key={action.id}
              className="mx-2 flex min-h-[60px] items-start gap-2 rounded-[10px] px-2 py-2"
            >
              <button
                type="button"
              ...
              <div className="min-w-0 flex-1 pt-1">
                ...
                <p className="mt-0.5 line-clamp-2 text-[11px] leading-snug text-[color:var(--text-secondary)]">
                  {action.action}
                </p>
              </div>
            </div>
